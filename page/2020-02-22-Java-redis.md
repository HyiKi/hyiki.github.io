...
在docker中安装的Redis需要手动拉取配置文件

```
docker pull redis:6.0-rc3-alpine3.11
```
...
```
docker run -p 6379:6379 --name redis-blog -v /root/redis.conf:/etc/redis/redis.conf -v /usr/local/docker/data:/data -d redis redis-server /etc/redis/redis.conf --appendonly yes
```
